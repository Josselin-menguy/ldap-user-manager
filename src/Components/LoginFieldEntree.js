 ...
 * Retourne un booléen : true s'il existe, false sinon.
 */
...
/**
 * Composant React pour la génération du login lors de la création d'un compte.
 * Il génère un login unique en partant de la première lettre du prénom + nom.
 * S'il existe déjà, il ajoute progressivement plus de lettres du prénom.
 * Si toutes les lettres du prénom sont utilisées, il ajoute un chiffre aléatoire.
 */
const LoginFieldEntree = ({
  firstName,
  lastName,
  ...
  useEffect(() => {
    const generateAndCheckLoginName = async () => {
      ...
      // Démarre avec la version de base (1 lettre du prénom)
      ...
      // Tant que le login existe, augmente le nombre de lettres du prénom à utiliser
      while (exists) {
        lettersToUse++;
  ...
  return (
    <div className="relative grid grid-cols-1 mt-4">
      <label htmlFor="loginFieldEntree" className="block text-sm font-medium text-gray-700 dark:text-gray-300">
...
export default LoginFieldEntree;
